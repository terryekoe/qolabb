...
export function TeamWorkloadWidget({
        ...
        for (const teamId of uniqueTeamIds) {
          try {
            ...
            const studentMembers = members.filter((member: any) => {
              const userRole = member.user?.role?.toLowerCase() || member.profile?.role?.toLowerCase() || '';
  ...
  const calculateWorkload = useMemo(() => {
    ...
    Object.entries(tasksByTeam).forEach(([teamId, teamTasks]) => {
      ...
      const totalAssignedTasks = Object.values(memberTaskCounts).reduce((sum, m) => sum + m.total, 0);
      ...
      members.forEach((member: any) => {
        ...
        const percentage = avgTasksPerMember > 0 
          ? Math.round((counts.total / avgTasksPerMember) * 100)
          : 0;
  ...
  const hasImbalance = sortedWorkload.length > 0 && (
  ...
  const redistributionSuggestions = useMemo(() => {
    ...
    const underutilized = sortedWorkload.filter(m => m.totalTasks < avgTasks * 0.7 && m.totalTasks < avgTasks);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className={cn(
              ...
              balanceScore >= 70 ? "bg-green-100 text-green-700 border border-green-200" :
              balanceScore >= 50 ? "bg-yellow-100 text-yellow-700 border border-yellow-200" :
              "bg-red-100 text-red-700 border border-red-200"
            )}>
              <div className={cn(
                "w-2 h-2 rounded-full",
                balanceScore >= 70 ? "bg-green-600" :
                balanceScore >= 50 ? "bg-yellow-600" :
                "bg-red-600"
              )} />
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 rounded-lg p-4 flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-yellow-800 mt-1">
                  Consider redistributing tasks to ensure equitable participation across team members.
                </p>
              </div>
            </div>
        ...
        <div className="mb-6 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-5 border border-gray-200">
          ...
          <div className="space-y-3">
            {sortedWorkload.slice(0, 8).map((member) => {
              ...
              const deviation = avgTasks > 0 ? ((member.totalTasks - avgTasks) / avgTasks) * 100 : 0;
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-amber-700 mt-0.5">
                          {statistics.unassignedCount} task{statistics.unassignedCount !== 1 ? 's' : ''} need assignment
                        </p>
                      </div>
                    </div>
