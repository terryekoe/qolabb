...
import { ClipboardCheck, CheckCircle2, Clock, Users, TrendingUp, AlertCircle, X } from 'lucide-react';
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (isInstructor || isAdmin || canAccess.instructorFeatures()) {
        loadPromises.push(
          getTeamEvaluationsForInstructor(user.id, currentWorkspace.id, undefined, selectedPeriodId || undefined)
        );
      }
    ...
    <DashboardLayout>
      <div className="h-full overflow-y-auto p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Peer Evaluations</h1>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">
                  {results?.averageScores?.overall
                    ? results.averageScores.overall.toFixed(1)
                    : '—'}
                </p>
              </div>
        ...
        {(isInstructor || isAdmin || canAccess.instructorFeatures()) && instructorEvaluations.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                <Users size={20} />
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Based on {results.totalEvaluations} evaluation{results.totalEvaluations !== 1 ? 's' : ''}
                  </p>
                  {results.evaluations?.[0]?.evaluation_period?.is_anonymous && (
