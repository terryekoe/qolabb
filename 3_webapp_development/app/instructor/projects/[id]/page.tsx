...
import { ArrowLeft, Users, CheckCircle2, Clock, AlertCircle, Search, Filter, X } from 'lucide-react';
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{stats.submitted}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{stats.graded}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    {!isEditingDetails ? (
                      <Button
                        ...
                        disabled={teamsData.some(t => t.submission) || (project.due_date && new Date(project.due_date) < new Date())}
                        title={teamsData.some(t => t.submission) ? "Cannot edit after submission" : (project.due_date && new Date(project.due_date) < new Date()) ? "Cannot edit after deadline" : "Edit assignment"}
                      >
                        <Pencil size={18} />
                      </Button>
                    ) : (
                      ...
                      <div>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              if (url && name) {
                                setEditForm({
                                  ...editForm,
                                  resources: [...editForm.resources, { id: crypto.randomUUID(), name, url, type: 'link' }]
                                });
                              }
                            }}
                            className="text-xs"
                        ...
                        <div className="space-y-2">
                            ...
                            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-700">
                              <div className="flex items-center gap-3 overflow-hidden">
                                ...
                                <div className="min-w-0">
                                  <div className="font-medium text-sm text-gray-900 dark:text-gray-100 truncate">{resource.name}</div>
                          ...
                          {editForm.resources.length === 0 && (
                            <p className="text-sm text-gray-500 italic text-center py-4">No resources added</p>
                          )}
                        </div>
                      </div>
                    </div>
                          ...
                          <div className="space-y-2">
                            {project.resources.map((resource: any) => (
                              ...
                              >
                                ...
                                <div>
                                  <div className="font-medium text-sm text-gray-900 dark:text-gray-100">{resource.name}</div>
                                  <div className="text-xs text-gray-500 truncate">{resource.url}</div>
                                </div>
                              </a>
                            ))}
